...
bool line_direction_change(int x1, int y1, int const x2, int const y2, int imr, int imc, Mat direction)
{
...
void points2KDtree(cv::Mat inter_lines_Mf, cv::Mat *inter_knn_Mi, cv::Mat lines_Mf, cv::Mat *lines_knn_Mi_1,
                   cv::Mat *lines_knn_Mi_2, int SUPPORT_POINT_NUM1, int SUPPORT_POINT_NUM2, SfMManager *sfm,
                   int imageID, float imr, float imc)
{
    ...
    flannIndex.knnSearch(inter_pts, inter_2_pt3index_M, inter_2_pt3index_dist, SUPPORT_POINT_NUM1,
    ...
    *inter_knn_Mi = cv::Mat(inter_pts.rows, SUPPORT_POINT_NUM1, CV_32SC1);
    ...
    for (int i = 0; i < inter_pts.rows; i++)
    {
        for (int j = 0; j < SUPPORT_POINT_NUM1; j++)
        {
            ...
            inter_knn_Mi_ptr[ind_inter] = pt2_idpt3__Mf.at<float>(inter_2_pt3index_M_ptr[ind_inter], 0) - 1;
    ...
    flannIndex.knnSearch(line_pts1, line_2_pt3index_1, line_2_pt3index_dist1, SUPPORT_POINT_NUM2,
                         cv::flann::SearchParams());

    cv::Mat line_2_pt3index_2;
    cv::Mat line_2_pt3index_dist2;
    cv::Mat line_pts2 = lines_Mf.colRange(2, 4).clone();
    flannIndex.knnSearch(line_pts2, line_2_pt3index_2, line_2_pt3index_dist2, SUPPORT_POINT_NUM2,
                         cv::flann::SearchParams());
    ...
    *lines_knn_Mi_2 = cv::Mat(lines_Mf.rows, SUPPORT_POINT_NUM2, CV_32SC1);
    ...
    for (int i = 0; i < lines_Mf.rows; i++)
        for (int j = 0; j < SUPPORT_POINT_NUM2; j++)
        {
            ...
            lines_knn_Mi_1_ptr[ind_ptr] = pt2_idpt3__Mf.at<float>(line_2_pt3index_1_ptr[ind_ptr], 0) - 1;

            lines_knn_Mi_2_ptr[ind_ptr] = pt2_idpt3__Mf.at<float>(line_2_pt3index_2_ptr[ind_ptr], 0) - 1;
    ...
    *queryRes = cv::Mat(queryPoints.rows, k, CV_32SC1);
    ...
    for (int i = 0; i < queryPoints.rows; i++)
    {
        for (int j = 0; j < k; j++)
        {
            ...
            // printf("%d\n",subInd);
            queryRes_ptr[ind_inter] = keyPoints_ind.at<float>(subInd, 0) - 1;
            ind_inter++;
        }
    }
    return true;
...
void maxMinDepth(int support_num, int lp_index, float *spacepoints, int *inter_knn, float *CM, float &mindepth,
                 float &maxdepth)
{
    mindepth = 99999;
    maxdepth = 0;
    ...
    for (int i = 0; i < support_num; i++)
    {
        ...
        w = CM[11] + CM[8] * spacepoints[indpt3] + CM[9] * spacepoints[indpt3 + 1] + CM[10] * spacepoints[indpt3 + 2];
...
void maxMinPt(int feature_size, int support_num, float *spacepoints, int *inter_knn, float *CM, float *out_range)
{
    ...
    for (int i = 0; i < feature_size; i++)
    {
        ...
        for (int j = 0; j < support_num; j++)
        {
            ...
            w = CM[11] + CM[8] * spacepoints[indpt3] + CM[9] * spacepoints[indpt3 + 1] +
...
{
    float M[] = {CM.at<float>(0, 0), CM.at<float>(0, 1), CM.at<float>(0, 2), CM.at<float>(1, 0), CM.at<float>(1, 1),
                 CM.at<float>(1, 2), CM.at<float>(2, 0), CM.at<float>(2, 1), CM.at<float>(2, 2)};
...
int processImage(SfMManager *sfm, int img_ind, float costhre, float dist, int inter_support_num, int maxwidth,
                 std::string lineType, int maxlineNum)
{
    ...
    std::string lineFile = sfm->outFolder + "\\" + lineType + "\\" + image_name + ".bin";
    cv::Mat lines_Mf;

    if (filesystem::exists(lineFile))
    {
        lines_Mf = loadMatBinary(lineFile);
    }
    else
    {
        ...
        saveMatBinary(lineFile, lines_Mf);
    }
    ...
    {
        int imr_2, imc_2;
        sfm->iImSize(img_ind, imr_2, imc_2);

        for (int i = 0; i < pt2_Mf.rows; i++)
        {
            pt2_Mf.at<float>(i, 0) = pt2_Mf.at<float>(i, 0) + imc_2 / 2.0;
            pt2_Mf.at<float>(i, 1) = pt2_Mf.at<float>(i, 1) + imr_2 / 2.0;
        }
    }

    cv::Mat inter_knn_Mi, line_knn_Mi_1, line_knn_Mi_2, linemid_knn;

    if (!points_knn(inter_lines_Mf.colRange(2, 4).clone(), pt2_Mf, pt2_idpt3__Mf, SUPPORT_POINT_NUM, &inter_knn_Mi))
        return 0;

    if (!points_knn(lines_Mf.colRange(0, 2).clone(), pt2_Mf, pt2_idpt3__Mf, SUPPORT_POINT_NUM, &line_knn_Mi_1))
        return 0;

    if (!points_knn(lines_Mf.colRange(2, 4).clone(), pt2_Mf, pt2_idpt3__Mf, SUPPORT_POINT_NUM, &line_knn_Mi_2))
        return 0;
    ...
    maxMinPt(inter_lines_Mf.rows, inter_knn_Mi.cols, sfm->points3D_ptr(), (int *)inter_knn_Mi.data,
             sfm->iCameraMatPtr(img_ind), (float *)inter_max_min.data);

    cv::Mat line_max_min_1 = cv::Mat(lines_Mf.rows, 2, CV_32FC1);
    cv::Mat line_max_min_2 = cv::Mat(lines_Mf.rows, 2, CV_32FC1);

    maxMinPt(lines_Mf.rows, line_knn_Mi_1.cols, sfm->points3D_ptr(), (int *)line_knn_Mi_1.data,
             sfm->iCameraMatPtr(img_ind), (float *)line_max_min_1.data);

    maxMinPt(lines_Mf.rows, line_knn_Mi_2.cols, sfm->points3D_ptr(), (int *)line_knn_Mi_2.data,
             sfm->iCameraMatPtr(img_ind), (float *)line_max_min_2.data);
