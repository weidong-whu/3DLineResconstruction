                ...
                float &x1, float &y1, float &x2, float &y2)
{
    ...
    pv[0] = (CM[5] * CM[10] * x - CM[6] * CM[9] * x - CM[1] * CM[10] * y + CM[2] * CM[9] * y + CM[1] * CM[6] * z -
             CM[2] * CM[5] * z) /
            (CM[0] * CM[5] * CM[10] - CM[0] * CM[6] * CM[9] - CM[1] * CM[4] * CM[10] + CM[1] * CM[6] * CM[8] +
             CM[2] * CM[4] * CM[9] - CM[2] * CM[5] * CM[8]);

    pv[1] = -(CM[4] * CM[10] * x - CM[6] * CM[8] * x - CM[0] * CM[10] * y + CM[2] * CM[8] * y + CM[0] * CM[6] * z -
              CM[2] * CM[4] * z) /
            (CM[0] * CM[5] * CM[10] - CM[0] * CM[6] * CM[9] - CM[1] * CM[4] * CM[10] + CM[1] * CM[6] * CM[8] +
             CM[2] * CM[4] * CM[9] - CM[2] * CM[5] * CM[8]);

    pv[2] = (CM[4] * CM[9] * x - CM[5] * CM[8] * x - CM[0] * CM[9] * y + CM[1] * CM[8] * y + CM[0] * CM[5] * z -
             CM[1] * CM[4] * z) /
            (CM[0] * CM[5] * CM[10] - CM[0] * CM[6] * CM[9] - CM[1] * CM[4] * CM[10] + CM[1] * CM[6] * CM[8] +
             CM[2] * CM[4] * CM[9] - CM[2] * CM[5] * CM[8]);
...
void makeCoffe(float *A, float *e, float *P, float *L, float *M, float *b, int cr)
{
    ...
    b[cr] = A[2] * L[0] + A[5] * L[1] + A[8] * L[2] + P[0] * (A[0] * L[0] + A[3] * L[1] + A[6] * L[2]) +
            P[1] * (A[1] * L[0] + A[4] * L[1] + A[7] * L[2]);
}

void obtainH(float *A, float *e, float *l11, float *l11f, float *l12, float *l21f, float *l22f, float *M, float *b,
             float *H, int &indd)

{
    ...
    v0 = -(M[1] * M[5] * b[2] - M[2] * M[4] * b[2] - M[1] * M[8] * b[1] + M[2] * M[7] * b[1] + M[4] * M[8] * b[0] -
           M[5] * M[7] * b[0]) /
         (M[0] * M[4] * M[8] - M[0] * M[5] * M[7] - M[1] * M[3] * M[8] + M[1] * M[5] * M[6] + M[2] * M[3] * M[7] -
          M[2] * M[4] * M[6]);

    v1 = (M[0] * M[5] * b[2] - M[2] * M[3] * b[2] - M[0] * M[8] * b[1] + M[2] * M[6] * b[1] + M[3] * M[8] * b[0] -
          M[5] * M[6] * b[0]) /
         (M[0] * M[4] * M[8] - M[0] * M[5] * M[7] - M[1] * M[3] * M[8] + M[1] * M[5] * M[6] + M[2] * M[3] * M[7] -
          M[2] * M[4] * M[6]);

    v2 = -(M[0] * M[4] * b[2] - M[1] * M[3] * b[2] - M[0] * M[7] * b[1] + M[1] * M[6] * b[1] + M[3] * M[7] * b[0] -
           M[4] * M[6] * b[0]) /
         (M[0] * M[4] * M[8] - M[0] * M[5] * M[7] - M[1] * M[3] * M[8] + M[1] * M[5] * M[6] + M[2] * M[3] * M[7] -
          M[2] * M[4] * M[6]);
...
void findHomography(float *inter_range, float *lpsm, int lpsm_size, float *lpsn, int lpsn_size, int *plMap, float *lm,
                    float *ln, cv::Mat CM_Mf, cv::Mat CN_Mf, cv::Mat Ae_Mf, cv::Mat F_Mf, float *M1Trans,
                    float *C1Trans, int imr, int imc, cv::Mat &match_H, cv::Mat &match_plane, cv::Mat &plane_line,
                    float error_max, float max_ang, int index)
{
    ...
    for (int i = 0; i < lpsm_size; i++)
    {
        ...
        depthRange((float *)C.data, CM, CN, itpm, principal_r, mindepth, maxdepth, x1, y1, x2, y2);
        ...
        for (int mm = 0; mm < xx.size(); mm++)
            for (int k = -buffer; k <= buffer; k++)
            {
                ...
                int indl2 = 0;
                obtainH((float *)A.data, (float *)e.data, l11, l11f, l12, l21f, l22f, M_3_3, b_3, H_3_3, indl2);
                ...
                if (indl2 == 0)
                {
                    equation_plane(pt3d1[0], pt3d1[1], pt3d1[2], pt3d2[0], pt3d2[1], pt3d2[2], pt3d3[0], pt3d3[1],
                                   pt3d3[2], vv);
                }
                else
                {
                    equation_plane(pt3d1[0], pt3d1[1], pt3d1[2], pt3d2[0], pt3d2[1], pt3d2[2], pt3d4[0], pt3d4[1],
                                   pt3d4[2], vv);
                }
    ...
    for (int i = 0; i < match_score_.rows; i++)
    {
        ...
        {
            if (match_score_.at<float>(i, 1) != match_score_.at<float>(j, 1))
                continue;

            if (match_score_.at<float>(i, 2) >= match_score_.at<float>(j, 2))
            {
                is_best = false;
                break;
            }
...
void findPLHomography(cv::Mat ppl1, cv::Mat ppl2, int *plMap, float *lm, float *ln, cv::Mat lmrange, cv::Mat CM_Mf,
                      cv::Mat CN_Mf, cv::Mat Ae_Mf, cv::Mat F_Mf, float *M1Trans, float *C1Trans, int imr, int imc,
                      cv::Mat &match_H, cv::Mat &match_plane, cv::Mat &plane_line, float error_max, float max_ang,
                      int index)
{
    ...
    for (int i = 0; i < ppl1.rows; i++)
    {
        ...
        for (int mm = 0; mm < xx.size(); mm++)
        {
            ...
            if (indl2 == 0)
            {
                equation_plane(pt3d1[0], pt3d1[1], pt3d1[2], pt3d2[0], pt3d2[1], pt3d2[2], pt3d3[0], pt3d3[1], pt3d3[2],
                               vv);
            }
            else
            {
                equation_plane(pt3d1[0], pt3d1[1], pt3d1[2], pt3d2[0], pt3d2[1], pt3d2[2], pt3d4[0], pt3d4[1], pt3d4[2],
                               vv);
            }
