...
{
    static float q[7] = {75122.6331530, 80916.6278952, 36308.2951477, 8687.24529705,
...
float nfa(int n, int k, float p, float logNT)
{
     ...
     */
    log1term = log_gamma((float)n + 1.0) - log_gamma((float)k + 1.0) - log_gamma((float)(n - k) + 1.0) +
    ...
    for (i = k + 1; i <= n; i++)
    {
         ...
         */
        bin_term =
            (float)(n - i + 1) * (i < TABSIZE ? (inv[i] != 0.0 ? inv[i] : (inv[i] = 1.0 / (float)i)) : 1.0 / (float)i);
