...
void crossPT(float *lines, int size, float costhre, float dist, std::vector<LP> &lps)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        for (j = i + 1; j < size; j++)
        {
            ...
            if (!LineLineIntersect(lines[ptri], lines[ptri + 1],     // Line 1 start
...
void callCrossPt(cv::Mat *lp_Mf, float *lines, int size, float costhre, float dist)
{
    ...
    *lp_Mf = cv::Mat(lpsize, 8, CV_32FC1);
    float *outlp = (float *)lp_Mf->data;
...
{
    float veca[2], vecb[2], linef[3], mid[2], intervec[2], intervec_mid[2], nearestPt[2];
    float norma, normb;

    float d1, d2;

    cv::Mat ppl;
    cv::Mat ppl_cell(1, 6, CV_32FC1);
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        mid[0] = (lines[ptri] + lines[ptri + 2]) / 2;
        mid[1] = (lines[ptri + 1] + lines[ptri + 3]) / 2;

        norma = normM(veca[0], veca[1]);

        cross_v2(lines + ptri, linef);
        ...
        for (int j = i + 1; j < size; j++)
        {
            ...
            normb = normM(vecb[0], vecb[1]);
            float cost = dot(veca[0], veca[1], vecb[0], vecb[1]) / (norma * normb);

            if (abs(cost) <= costhre)
                continue;

            // distance
            d1 = point_2_line_dis(lines + ptrj, linef);
            if (d1 < distmin || d1 > distmax)
                continue;

            d2 = point_2_line_dis(lines + ptrj + 2, linef);
            if (d2 < distmin || d2 > distmax)
                continue;

            // intersection
            intervec[0] = lines[ptrj] - lines[ptrj + 2];
            intervec[1] = lines[ptrj + 1] - lines[ptrj + 3];
            intervec_mid[0] = mid[0] - lines[ptrj + 2];
            intervec_mid[1] = mid[1] - lines[ptrj + 3];

            if (intervec[0] * intervec_mid[0] + intervec[1] * intervec_mid[1] < 0)
                continue;

            intervec[0] = lines[ptrj + 2] - lines[ptrj];
            intervec[1] = lines[ptrj + 3] - lines[ptrj + 1];
            intervec_mid[0] = mid[0] - lines[ptrj];
            intervec_mid[1] = mid[1] - lines[ptrj + 1];

            if (intervec[0] * intervec_mid[0] + intervec[1] * intervec_mid[1] < 0)
                continue;

            if (dot_v2(veca[0], veca[1], vecb[0], vecb[1]) < 0)
            {
                ppl_cell.at<float>(0, 0) = i;
                ppl_cell.at<float>(0, 1) = j;
            }
            else
            {
                ppl_cell.at<float>(0, 0) = j;
                ppl_cell.at<float>(0, 1) = i;
            }

            closestPointAlongLine(lines + ptri, veca, lines + ptrj, nearestPt);
            ppl_cell.at<float>(0, 2) = (lines[ptrj] + nearestPt[0]) / 2;
            ppl_cell.at<float>(0, 3) = (lines[ptrj + 1] + nearestPt[1]) / 2;

            closestPointAlongLine(lines + ptri, veca, lines + ptrj + 2, nearestPt);
            ppl_cell.at<float>(0, 4) = (lines[ptrj + 2] + nearestPt[0]) / 2;
            ppl_cell.at<float>(0, 5) = (lines[ptrj + 3] + nearestPt[1]) / 2;

            ppl.push_back(ppl_cell.clone());
            /*
            printf
            ("%f %f %f %f %f %f %f %f %f %f %f %f\n",
                lines[ptri], lines[ptri + 1], lines[ptri + 2], lines[ptri + 3],
                lines[ptrj], lines[ptrj + 1], lines[ptrj + 2], lines[ptrj + 3],
                ppl_cell.at<float>(0, 2),
                ppl_cell.at<float>(0, 3),
                ppl_cell.at<float>(0, 4),
                ppl_cell.at<float>(0, 5)
            );
            */
        }
    }
