	...
	int imr, int imc)
{
	...
	for (int i = 0; i < hom_size; i++)score_plane[i] = 0;
	...
	for (int i = 0; i < lsize1; i++)
	{
		...
		if (!tringulate3Dline(
			...
			M1, C1,
			l1,
			(float*)pt3Ds.data, (float*)pt3Ds.data + 3))
			continue;
	...
	for (int i = 0; i < hom_size; i++)
	{
		...
		for (int j = i + 1; j < hom_size; j++)
		{
			...
			if (!(
				(planes_line_i[0] == planes_line_j[0] && planes_line_i[1] != planes_line_j[1]) ||
	...
	for (int i = 0; i < match_plane_id.size(); i++)
	{
		...
		{
			if(matchs_tempall.at<ushort>(i, 2)!=
			   matchs_tempall.at<ushort>(j, 2))
				continue;
...
void line2KDtree(cv::Mat lines_Mf, cv::Mat homo_Mf, cv::Mat* inter_knn_Mi, int support_H_num)
{
	...
	flannIndex.knnSearch(lines_Mf.colRange(4, 6).clone(), *inter_knn_Mi,
	...
	float ang_max)
{
	...
	if (sfm->iJunctionLines(mid1)->rows >= 1 && sfm->iJunctionLines(mid2)->rows >= 1)
		findHomography(
			(float*)inter_range_i.data,
			(float*)l2l_1_Mf.data, l2l_1_Mf.rows,
			(float*)l2l_2_Mf.data, l2l_2_Mf.rows,
			(int*)l2l_Mf.data,
			(float*)lines1_Mf.data, (float*)lines2_Mf.data,
			CM, CN, Ae, F, M1, C1,
			rm2, cm2, homos_junc, planes_junc, planes_lineid_junc,
			error_max, ang_max, match_ind);
	...
	if (parra_Lines1.rows >= 1 && parra_Lines2.rows >= 1)
	{
		...
		findPLHomography
		(
			parra_Lines1, parra_Lines2, (int*)parral_map.data,
			(float*)lines1_Mf.data, (float*)lines2_Mf.data, line_range_i,
			CM, CN, Ae, F, M1, C1,
			rm2, cm2, homos_parra, planes_parra, planes_lineid_parra,
			error_max, ang_max, match_ind
		);
	}
	...
	guidedMatching(match, mid1, mid2, match_ind,
		(float*)lines1_Mf.data, (float*)line_range_i.data,
		(int*)line_knn_Mi.data, (float*)lines2_Mf.data,
		l2l_Mf, lines1_Mf.rows, lines2_Mf.rows,
		(float*)homos.data, (float*)planes.data, (ushort*)planes_lineid.data, homos.rows,
		CM, CN, F,
		M1, C1, M2, C2,
		line_knn_Mi.cols, dist,
		rm2, cm2);
