...
static  void extractAnchor(Mat ang, Mat grad, Mat& anchor, struct coorlist** list_p, void** mem_p)
{
	...
	/* get memory for "ordered" list of pixels */
	list = (struct coorlist*)malloc((size_t)(m * n)*sizeof(struct coorlist));
	*mem_p = (void *) list;
	range_l_s = (struct coorlist**)malloc(((size_t)n_bins)*
		sizeof(struct coorlist*));
	range_l_e = (struct coorlist**)malloc(((size_t)n_bins) *
		sizeof(struct coorlist*));
...
static bool densityVlidation(RegManager* rm, BaseLine* bl, uchar* used, uchar* anchor, float& thisdthre)
{
	...
	int lastcounter = 0, i;
	rm->counter = rm->ed4 - rm->ed3 + rm->ed2 - rm->ed1;
	while (1)
	{
...
int main1(string picstr, string fname)
{
	...
	extractAnchor(angM, gradM, anchorM, &list_p,&mem_p);
	//finish = clock();
...
int ag3line(Mat& im, vector<lineag>& lines, bool control)
{
	...
	int minlength = 20;// -4 * std::log(N) / std::log(0.125);
	int minregsize = minlength * 0.6;
	...
	for (; list_p != NULL; list_p = list_p->next)
	{
		...
		float segang=bl.theta;

		float dx = bl.x2 - bl.x1;
		float dy = bl.y2 - bl.y1;
		float at2 = atan2approx(dy, dx);

		if (angle_diff(bl.theta, at2) < prec)
		{
			lg.x1 = bl.x1;
			lg.y1 = bl.y1;
			lg.x2 = bl.x2;
...
{
	bool control = false;
	string imgstr = "81551_150560149_038_20160205055916_rgb.jpg";
