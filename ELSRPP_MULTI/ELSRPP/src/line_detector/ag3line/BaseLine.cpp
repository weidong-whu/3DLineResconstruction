#include "line_detector/ag3line/BaseLine.h"
#include "line_detector/ag3line/GeneralFuncs.h"
...
void BaseLine::getLine(Point3i*reg)
{
...
double BaseLine::get_theta( Point3i *reg, int ed1,int ed2,int ed3,int ed4, double x, double y,
...
void BaseLine::region2line( Point3i *reg, int ed1,int ed2,int ed3,int ed4,
                         double prec)
{
...
{
	xxs[0]=round_my((int)sx+(float)i*dx);
	yys[0]=round_my((int)sy+(float)i*dy);
...
float BaseLine::distance_global(float x, float y)
{
	return std::abs(ui * x + uj * y + uk) / sqrtab;
}
...
float BaseLine::xy_error(float s_x2, float s_y2, float s_xy, float s_x, float s_y)
{
	float a_2_b_2;
	a_2_b_2 = ui * ui + uj * uj;
	...
	float err =
		(ui * ui / a_2_b_2) * s_x2 +
		((2 * ui * uj) / a_2_b_2) * s_xy +
		((2 * ui * uk) / a_2_b_2) * s_x +
		(uj * uj / a_2_b_2) * s_y2 +
		((2 * uj * uk) / a_2_b_2) * s_y +
		(uk * uk / a_2_b_2) * counter;

	return sqrt(err/ counter);
}