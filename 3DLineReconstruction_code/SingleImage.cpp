...
{
	int query_nb = queryPoints.cols;
	float* knn_dist1 = new float[query_nb * k];
	int* knn_index1 = new int[query_nb * k];

	if (!knn_cuda_global((float*)(keyPoints.data), keyPoints.cols,
		(float*)(queryPoints.data), queryPoints.cols,
		2, k, knn_dist1, knn_index1)) {
		printf("wrong in cuda knn\n");
		free(knn_dist1);
		free(knn_index1);
	...
	*queryRes = cv::Mat(queryPoints.cols, k, CV_32SC1);
	int* inter_knn_Mi_ptr = (int*)(*queryRes).data;

	int ind_inter = 0;
	int subInd = 0;
	for (int i = 0; i < queryPoints.cols; i++)
	{
		for (int j = 0; j < k; j++)
		{
			subInd = knn_index1[j * queryPoints.cols + i];
			//printf("%d\n",subInd);
			inter_knn_Mi_ptr[ind_inter]
...
bool points2KDtreeCuda(cv::Mat inter_lines_Mf, cv::Mat* inter_knn_Mi, cv::Mat lines_Mf,
	cv::Mat* lines_knn_Mi_1, cv::Mat* lines_knn_Mi_2,
	int SUPPORT_POINT_NUM1, int SUPPORT_POINT_NUM2,
	SfMManager* sfm, int imageID, float imr, float imc)
{
	...
	if (pt2.rows < SUPPORT_POINT_NUM1)
	{
		SUPPORT_POINT_NUM1 = pt2.rows;
	}

	if (pt2.rows < SUPPORT_POINT_NUM2)
	{
		SUPPORT_POINT_NUM2 = pt2.rows;
	}

	if (SUPPORT_POINT_NUM1 == 0 || SUPPORT_POINT_NUM2 == 0)
	{
		printf("non connection points\n");
		return false;

	}
	//printf("points: %d\n", pt2.rows);
	pt2_Mf = pt2.colRange(0, 2).t();
	...
	for (int i = 0; i < pt2_Mf.cols; i++)
	{
		pt2_Mf.at<float>(0, i) = pt2_Mf.at<float>(0, i) + imc_2;
		pt2_Mf.at<float>(1, i) = pt2_Mf.at<float>(1, i) + imr_2;
	}

	cv::Mat inter_pts = inter_lines_Mf.colRange(2, 4).clone().t();
	int query_nb = inter_pts.cols;
	int k1 = SUPPORT_POINT_NUM1;

	float* knn_dist1 = (float*)malloc(query_nb * k1 * sizeof(float));
	int* knn_index1 = (int*)malloc(query_nb * k1 * sizeof(int));

	if (!knn_cuda_global((float*)(pt2_Mf.data), pt2_Mf.cols,
		(float*)(inter_pts.data), inter_pts.cols,
		2, k1, knn_dist1, knn_index1)) {
		free(knn_dist1);
		free(knn_index1);
		printf("wrong in cuda knn\n");
		return false;
	...
	*inter_knn_Mi = cv::Mat(inter_pts.cols, SUPPORT_POINT_NUM1, CV_32SC1);
	int* inter_knn_Mi_ptr = (int*)(*inter_knn_Mi).data;

	int ind_inter = 0;
	int subInd = 0;
	for (int i = 0; i < inter_pts.cols; i++)
	{
		for (int j = 0; j < SUPPORT_POINT_NUM1; j++)
		{
			subInd = knn_index1[j * inter_pts.cols + i];
			//printf("%d\n",subInd);
			inter_knn_Mi_ptr[ind_inter]
				= pt2_idpt3__Mf.at<float>(subInd, 0) - 1;
	...
	// may be the clone is useless, which need to be tested;
	// line segment KD tree 
	cv::Mat line_pts1 = lines_Mf.colRange(0, 2).clone().t();
	query_nb = line_pts1.cols;
	int k2 = SUPPORT_POINT_NUM2;

	float* knn_dist2 = (float*)malloc(query_nb * k2 * sizeof(float));
	int* knn_index2 = (int*)malloc(query_nb * k2 * sizeof(int));

	if (!knn_cuda_global((float*)(pt2_Mf.data), pt2_Mf.cols,
		(float*)(line_pts1.data), line_pts1.cols,
		2, k2, knn_dist2, knn_index2)) {
		free(knn_dist2);
		free(knn_index2);
		printf("wrong in cuda knn\n");
		return false;
	...
	//store
	*lines_knn_Mi_1 = cv::Mat(line_pts1.cols, SUPPORT_POINT_NUM2, CV_32SC1);
	int* lines_knn_Mi_1_ptr = (int*)lines_knn_Mi_1->data;

	int  ind_ptr = 0;
	for (int i = 0; i < line_pts1.cols; i++)
		for (int j = 0; j < SUPPORT_POINT_NUM2; j++)
		{
			subInd = knn_index2[j * line_pts1.cols + i];
			lines_knn_Mi_1_ptr[ind_ptr] =
				pt2_idpt3__Mf.at<float>(subInd, 0) - 1;
			//-1 is aligned to vsfm.m file
			ind_ptr++;
	...
	// the second point 
	cv::Mat line_pts2 = lines_Mf.colRange(2, 4).clone().t();
	query_nb = line_pts2.cols;
	k2 = SUPPORT_POINT_NUM2;

	if (!knn_cuda_global((float*)(pt2_Mf.data), pt2_Mf.cols,
		(float*)(line_pts2.data), line_pts2.cols,
		2, k2, knn_dist2, knn_index2)) {
		free(knn_dist2);
		free(knn_index2);
		printf("wrong in cuda knn\n");
	...
	//store
	*lines_knn_Mi_2 = cv::Mat(line_pts2.cols, SUPPORT_POINT_NUM2, CV_32SC1);
	int* lines_knn_Mi_2_ptr = (int*)lines_knn_Mi_2->data;

	ind_ptr = 0;
	for (int i = 0; i < line_pts2.cols; i++)
		for (int j = 0; j < SUPPORT_POINT_NUM2; j++)
		{
			subInd = knn_index2[j * line_pts2.cols + i];
			lines_knn_Mi_2_ptr[ind_ptr] =
				pt2_idpt3__Mf.at<float>(subInd, 0) - 1;
			//-1 is aligned to vsfm.m file
			ind_ptr++;
	...
	cv::Mat mid(lines_Mf.rows, 2, CV_32FC1);
	float* midptr = (float*)mid.data;
	float* lines_Mfptr = (float*)lines_Mf.data;
	for (int i = 0; i < lines_Mf.rows; i++)
	{
		midptr[0] = (lines_Mfptr[0] + lines_Mfptr[1]) / 2;
		midptr[1] = (lines_Mfptr[2] + lines_Mfptr[3]) / 2;
		lines_Mfptr = lines_Mfptr + lines_Mf.cols;
		midptr = midptr + 2;
	}


	cv::Mat mid_trans = mid.t();
	query_nb = line_pts2.cols;
	k2 = SUPPORT_POINT_NUM1;

	if (!knn_cuda_global((float*)(pt2_Mf.data), pt2_Mf.cols,
		(float*)(mid_trans.data), mid_trans.cols,
		2, k2, knn_dist2, knn_index2)) {
		free(knn_dist2);
		free(knn_index2);
		printf("wrong in cuda knn\n");
	...
	std::string lineFolder, std::string lineExt)
{
	...
	*/
	...
	else if (uselsd == 2)
	{
		...
		write2txt((float*)lines_Mf.data, lines_Mf.rows, lines_Mf.cols, input_folder + "/" + image_name + ".agline");
	}
	else if (uselsd == 3)
	{
	...
	//printf("points: %d\n", pt2.rows);
	cv::Mat pt2_Mf = pt2.colRange(0, 2).t();
	...
	if (!points_knn(inter_lines_Mf.colRange(2, 4).clone().t(),
		pt2_Mf, pt2_idpt3__Mf, SUPPORT_POINT_NUM, &inter_knn_Mi))
		return 0;

	if (!points_knn(lines_Mf.colRange(0, 2).clone().t(),
		pt2_Mf, pt2_idpt3__Mf, SUPPORT_POINT_NUM, &line_knn_Mi_1))
		return 0;

	if (!points_knn(lines_Mf.colRange(2, 4).clone().t(),
