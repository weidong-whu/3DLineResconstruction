...
__global__ void compute_distances(float * ref,
    ...
    __shared__ volatile float shared_A[BLOCK_DIM][BLOCK_DIM];
    __shared__ volatile float shared_B[BLOCK_DIM][BLOCK_DIM];

    // Sub-matrix of A (begin, step, end) and Sub-matrix of B (begin, step)
    __shared__ volatile int begin_A;
    __shared__ volatile int begin_B;
    __shared__ volatile int step_A;
    __shared__ volatile int step_B;
    __shared__ volatile int end_A;
...
__global__ void add_reference_points_norm(float * array, int width, int pitch, int height, float * norm){
    ...
    __shared__ volatile float shared_vec[16];
